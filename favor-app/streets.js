...
function match(rows, address) {
  ...
  // An approved entry with no suffix ("Aiken") covers every suffix: Aiken Drive, Aiken Court, Aiken Lane...
  if (!hit) hit = rows.find((r) => !SUFFIX[r.key.split(" ").pop()] && p.key.startsWith(r.key + " "));
  if (!hit && !p.hasSuffix) {
