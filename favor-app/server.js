...
app.use("/api", (req, res, next) => {
  if (["POST", "PUT", "PATCH", "DELETE"].includes(req.method)) {
    ...
    const isImageUpload = req.method === "PUT" && /^\/posts\/\d+\/image$/.test(req.path) && req.is("image/*");
    if (hasBody && !isImageUpload && !req.is("application/json")) return res.status(415).json({ error: "Send JSON." });
    if (PROD) {
...
async function fullMe(user) {
  ...
  return {
    user: {
      ...
      isAdmin: isAdmin(user), banner: user.banner || 0, request: lastReq.rows[0] || null,
...
app.put("/api/skills/:id", requireOnboarded, async (req, res) => {
...
/* ---------------- posts ---------------- */
const postOut = (p) => ({ id: p.id, userId: p.user_id, text: p.text, createdAt: p.created_at, likes: p.likes || 0, liked: !!p.liked, hasImage: !!p.has_image });
...
app.post("/api/admin/wipe", requireAdmin, async (req, res) => {
  ...
  if (b.scope === "content") await db.query("TRUNCATE post_likes, post_images, posts, listings, messages, reports RESTART IDENTITY");
